    ...
    if ((context->currentField == TX_RLP_DATA) &&
        ...
        if (context->currentFieldPos == 0) {
            ...
            if ((context->currentFieldLength == TOKEN_TRANSFER_DATA_SIZE) &&
              ...
              contractProvisioned = CONTRACT_TOKEN;
            }
            else
...
void finalizeParsing(bool direct) {
  ...
  cx_hash((cx_hash_t *)&sha3, CX_LAST, tmpCtx.transactionContext.hash, 0, tmpCtx.transactionContext.hash, 32);
    ...
    if ((contractProvisioned == CONTRACT_TOKEN) || (contractProvisioned == CONTRACT_ALLOWANCE)) {
  ...
  // Add address
  if (tmpContent.txContent.destinationLength != 0) {
    getLegacyAccountAddressStringFromBinary(tmpContent.txContent.destination, address, &sha3);
    /*
    addressSummary[0] = '0';
    addressSummary[1] = 'x';
  ...
  if ((contractProvisioned == CONTRACT_NONE) || (contractProvisioned == CONTRACT_TOKEN) ||
    (contractProvisioned == CONTRACT_ALLOWANCE)) {
  ...
  adjustDecimals((char *)(G_io_apdu_buffer + 100), i, (char *)G_io_apdu_buffer, 100, PEB_TO_KLAY);
  i = 0;
  tickerOffset=0;
