...
void getLegacyAccountAddressFromKey(cx_ecfp_public_key_t *publicKey, uint8_t *out,
...
void getLegacyAccountAddressStringFromKey(cx_ecfp_public_key_t *publicKey, uint8_t *out,
                                cx_sha3_t *sha3Context) {
    uint8_t hashAddress[32];
    cx_keccak_init(sha3Context, 256);
    cx_hash((cx_hash_t*)sha3Context, CX_LAST, publicKey->W + 1, 64, hashAddress, 32);
    getLegacyAccountAddressStringFromBinary(hashAddress + 12, out, sha3Context);
}

void getLegacyAccountAddressStringFromBinary(uint8_t *address, uint8_t *out,
                                   cx_sha3_t *sha3Context) {
